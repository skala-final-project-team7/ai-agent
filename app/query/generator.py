...
def _agent_sources_to_rag_sources(
    *,
    ...
    download_url_by_chunk_id = _download_urls_by_chunk_id(top_chunks, existing_sources or [])
    sources: list[Source] = []
    for agent_source in agent_sources:
        ...
        sources.append(
            Source(
                ...
                download_url=download_url_by_chunk_id.get(metadata.chunk_id),
            )
        )
    return sources


def _download_urls_by_chunk_id(
    top_chunks: list[Chunk],
    existing_sources: list[Source],
) -> dict[str, str]:
    """rerank 단계 Source.download_url 을 generator 재구성 후에도 보존한다."""
    urls: dict[str, str] = {}
    for chunk, source in zip(top_chunks, existing_sources, strict=False):
        if source.download_url:
            urls[chunk.metadata.chunk_id] = source.download_url
    return urls
