...
class DeltaSyncRequest:
    ...
    # api-spec v2.5.0 preferred credential lookup key.
...
@dataclass
class DeltaSoftDeleteResult:
    """삭제 후보 soft-delete 적용 결과.

    ``run_delta_sync`` 는 삭제 후보를 자동 삭제하지 않고 surface 만 한다. 호출자(Sync Worker,
    운영 스케줄러, 관리자 승인 흐름)가 후보를 확정한 뒤 본 결과를 사용해 성공/실패를 기록한다.
    """
...
class _SoftDeletePageStore(Protocol):
    """삭제 후보를 soft-delete 할 수 있는 저장소 최소 계약."""

    def soft_delete_by_page_id(self, page_id: str) -> None:
        """``page_id`` 기준으로 관련 Qdrant points 를 soft-delete 처리한다."""
...
def soft_delete_delta_candidates(
    result: DeltaSyncResult,
    *,
    store: _SoftDeletePageStore,
) -> DeltaSoftDeleteResult:
    """Delta Sync 삭제 후보를 Qdrant soft-delete 로 확정 적용한다.

    이 함수는 명시적으로 호출될 때만 삭제 후보를 반영한다. ``run_delta_sync`` 단계에서 자동으로
    호출하지 않는 이유는, 현재 vendored Data Sync Agent 의 삭제 결과가
    ``deleted_candidate``(확정 전) 의미를 갖기 때문이다. 운영 정책상 Trash API/Webhook/
    관리자 승인/주기 Reconciliation 등 어떤 신호를 확정 근거로 삼을지 결정한 호출자가 이
    함수를 실행해야 한다.

    Args:
        result: ``run_delta_sync`` 결과. ``deleted_candidate_page_ids`` 를 읽는다.
        store: ``soft_delete_by_page_id`` 를 제공하는 Qdrant store.

    Returns:
        soft-delete 성공/실패 page_id 목록. 단일 page 실패가 나머지 후보 처리를 막지 않도록
        실패를 수집하고 계속 진행한다.
    """
    applied = DeltaSoftDeleteResult()
    for page_id in result.deleted_candidate_page_ids:
        try:
            store.soft_delete_by_page_id(page_id)
        except Exception:  # noqa: BLE001 — 후보 1건 실패가 전체 sync 후처리를 막지 않게 격리.
            applied.failed_page_ids.append(page_id)
            continue
        applied.soft_deleted_page_ids.append(page_id)
    return applied
