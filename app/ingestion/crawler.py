...
def _collect_attachment(
    ...
    *,
    ...
    try:
        ...
        publisher.publish(
            routing_key=QUEUE_ATTACHMENT,
            message=build_attachment_extraction_message(page, attachment),
        )
...
def build_attachment_extraction_message(
    page: PageObject, attachment: Attachment
) -> dict[str, object]:
    """첨부 Extraction Queue 발행 메시지 — raw_attachments 조회용 식별자만 전달한다."""
    return {
        "page_id": page.page_id,
        "attachment_id": attachment.attachment_id,
        "space_key": page.space_key,
