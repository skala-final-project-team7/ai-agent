...
from app.ingestion.attachment_downloader import AttachmentDownloader, DefaultAttachmentDownloader
...
class PipelineResult:
    ...
    crawl: CrawlResult
    extracted: list[AttachmentExtractionResult]
...
class PocComponents:
    ...
    attachment_deps: AttachmentExtractionDeps
...
def run_ingestion_pipeline(
    ...
    *,
    ...
    publisher: FakeQueuePublisher,
    chunking_deps: ChunkingWorkerDeps,
    attachment_deps: AttachmentExtractionDeps | None = None,
) -> PipelineResult:
    ...
    attachment_messages = [
        message.body for message in publisher.messages if message.routing_key == QUEUE_ATTACHMENT
    ]
    extracted: list[AttachmentExtractionResult] = []
    if attachment_messages and attachment_deps is not None:
        attachment_deps.publisher = publisher
        attachment_consumer = FakeMessageConsumer(messages=attachment_messages)
        extracted = run_attachment_extraction_worker(attachment_consumer, attachment_deps)
    ...
    return PipelineResult(crawl=crawl, extracted=extracted, indexed=indexed)
...
def build_poc_components(
    *,
    ...
    attachment_deps = AttachmentExtractionDeps(
        raw_store=raw_store,
        downloader=attachment_downloader or DefaultAttachmentDownloader(),
        publisher=publisher,
        jobs=jobs,
        chunking_routing_key=QUEUE_CHUNKING,
    )
...
def run_poc_ingestion(
    ...
    *,
    ...
    components = build_poc_components(
        doc_type_resolver=doc_type_resolver,
        chunk_attachment_fn=chunk_attachment_fn,
    ...
    result = run_ingestion_pipeline(
        ...
        attachment_deps=components.attachment_deps,
    )
