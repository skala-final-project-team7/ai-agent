...
def test_attachment_download_url_preserved_from_rerank_sources() -> None:
    chunks = [
        _make_chunk(
            chunk_id="chunk-1",
            page_title="첨부 자료",
    ...
    ctx_id_1 = f"ctx-001-{chunks[0].metadata.chunk_id[:8]}"
    fake = FakeAnswerLLMProvider(response=_fake_response(citations=[ctx_id_1]))
    ...
    state.sources = [
        Source(
            ...
            last_modified=datetime(2026, 5, 1, 9, 0, 0),
            text_preview="첨부 미리보기",
            attachment_filename="manual.pdf",
            attachment_mime="application/pdf",
            download_url="https://confluence.example.com/download/manual.pdf",
        )
    ]

    result = manage_generator(state, provider=fake)

    assert result.sources[0].download_url == "https://confluence.example.com/download/manual.pdf"
    assert result.sources[0].title == "manual.pdf"
