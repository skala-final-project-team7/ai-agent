...
def test_attachment_source_to_bff_payload_includes_attachment_fields() -> None:
    src = Source(**_ATTACHMENT_SOURCE, page_id="12345", space_id="98310", space_name="FINOPS")

    payload = src.to_bff_payload()

    assert payload["sourceType"] == "attachment"
    assert payload["attachmentFilename"] == "prod_cost_2026Q1.xlsx"
    assert payload["attachmentMime"] == (
        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )
    assert payload["downloadUrl"] == (
        "https://confluence/download/attachments/99001/prod_cost_2026Q1.xlsx"
    )
