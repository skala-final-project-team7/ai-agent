...
from app.ingestion.bootstrap import (
    build_attachment_extraction_deps,
...
def test_build_attachment_extraction_deps_wires_download_and_routing_key() -> None:
    shared = FakeRawPageStore()
    ...
    deps = build_attachment_extraction_deps(
        ...
        raw_store=shared,
        publisher=publisher,
        chunking_routing_key="custom.content.chunking",
    )

    assert deps.raw_store is shared
    assert deps.publisher is publisher
    assert deps.chunking_routing_key == "custom.content.chunking"
