...
class _SoftDeleteStore:
    def __init__(self, *, fail_page_ids: set[str] | None = None) -> None:
        ...
        self.soft_deleted_page_ids: list[str] = []

    def soft_delete_by_page_id(self, page_id: str) -> None:
        if page_id in self.fail_page_ids:
            raise RuntimeError(f"soft delete failed: {page_id}")
        self.soft_deleted_page_ids.append(page_id)


def test_soft_delete_delta_candidates_applies_page_soft_delete_only_when_called() -> None:
    store = _SoftDeleteStore()
    result = DeltaSyncResult(
        changed_pages=0,
        deleted_candidate_page_ids=["page-3", "page-9"],
        failed_items=0,
    )

    applied = soft_delete_delta_candidates(result, store=store)

    assert store.soft_deleted_page_ids == ["page-3", "page-9"]
    assert applied.soft_deleted_page_ids == ["page-3", "page-9"]
    assert applied.failed_page_ids == []


def test_soft_delete_delta_candidates_isolates_single_page_failure() -> None:
    store = _SoftDeleteStore(fail_page_ids={"page-err"})
    result = DeltaSyncResult(
        changed_pages=0,
        deleted_candidate_page_ids=["page-ok", "page-err", "page-next"],
        failed_items=0,
    )

    applied = soft_delete_delta_candidates(result, store=store)

    assert store.soft_deleted_page_ids == ["page-ok", "page-next"]
    assert applied.soft_deleted_page_ids == ["page-ok", "page-next"]
    assert applied.failed_page_ids == ["page-err"]
