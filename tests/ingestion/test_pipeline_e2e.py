...
class _FakeAttachmentDownloader:
    content: bytes = _VALID_ATTACH_CSV.encode("utf-8")
...
def test_end_to_end_crawl_to_attachment_chunk_upsert() -> None:
    ...
    result, components = run_poc_ingestion(
        ...
        source,
        chunk_attachment_fn=_fake_chunk_attachment,
        attachment_downloader=_FakeAttachmentDownloader(),
    )

    # crawl: 본문 1 + 첨부 1 수집·발행.
    assert result.crawl.pages_collected == 1
    assert result.crawl.attachments_collected == 1
    assert [r.status for r in result.extracted] == [IngestionStatus.SUCCESS]
    # 색인: 본문 메시지 + 첨부 추출 후 발행된 첨부 메시지 = 2건 모두 SUCCESS.
