...
## POST /ml/ingest  (BFF → ML, legacy HTTP trigger)
...
- 인증/credential: api-spec v2.5.0 preferred 운영 경로에서는 BFF/RabbitMQ payload에
  `accessToken`/`refreshToken`/`cloudId`를 넣지 않는다. Data Ingestion Worker가
  `adminUserId`로 auth-server 내부 credential API를 호출해 admin OAuth `accessToken` +
  `cloudId`를 조회한다.
- Legacy HTTP 호환: backend OAuth/RabbitMQ 연동 전 local/PoC smoke를 위해 `accessToken` /
  `cloudId` optional 입력은 유지한다. 이 값은 로그/응답/큐 메시지/completion event에 남기지 않는다.
- Admin Key 수명주기: ML은 Admin Key를 직접 revoke하지 않고 BFF HTTP callback도 호출하지 않는다.
  Background job 종료 후 RabbitMQ completion event(`jobId`/`adminUserId`/`mode`/`status`/
  `completedAt`/`errorCode`/`message`)를 발행한다. BFF consumer가 event를 consume하고
  auth-server `POST /internal/admin/key/deactivate`를 호출한다.
...
| `adminUserId` | string | N | v2.5 preferred. auth-server 내부 credential 조회 key |
| `accessToken` | string | N | Legacy PoC-only. Confluence OAuth access token. 로그/응답/큐/event 금지 |
| `cloudId` | string | N | Legacy PoC-only. Confluence cloud id. 큐/event 금지 |
...
- `COMPLETED` 또는 `FAILED` 기록 직후 ingestion completion event를 발행한다.
- completion event에는 `accessToken`, `refreshToken`, `cloudId`를 포함하지 않는다.
...
- `json_fixture` + fake/in-memory adapter로 `/ml/ingest` route를 통과한다.
- 외부 Confluence/Admin Key/RabbitMQ completion event는 호출하지 않는다.
- 실 Confluence smoke는 `RAG_SOURCE_TYPE=atlassian`, `RAG_ATLASSIAN_*`, RabbitMQ,
  auth-server 내부 credential lookup 구성을 주입한 환경에서 별도로 수행한다.
