...
- ML 서버는 BFF가 전달한 `userId`/`groups`/`spaceKey`와 수집 단계 토큰만 사용한다.
...
Confluence space 수집을 비동기로 트리거한다. `mode="full"`은 Data Ingestion Agent 기반 full
...
| 필드 | 타입 | 필수 | 설명 |
|---|---|---|---|
| `spaceKey` | string | Y | 수집 대상 Confluence space key |
