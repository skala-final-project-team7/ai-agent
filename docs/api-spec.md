...
- 외부 Confluence/Admin Key/BFF callback은 호출하지 않는다.
- 실 Confluence smoke는 `RAG_SOURCE_TYPE=atlassian`, `RAG_ATLASSIAN_*`,
  `RAG_BFF_ADMIN_KEY_REVOKE_URL` 등 운영 설정을 주입한 환경에서 별도로 수행한다.
