...
- [~] `AtlassianSourceAdapter` 운영 ACL 보강 — Admin Key header 설정(`RAG_ATLASSIAN_USE_ADMIN_KEY`) + `restriction/byOperation/read` 조회 client + read restriction 응답 → `allowed_users`/`allowed_groups` 매핑 seam 구현 완료. Confluence group 식별자 우선순위(`RAG_ATLASSIAN_GROUP_ACL_FIELD_ORDER`)와 prefix(`RAG_ATLASSIAN_GROUP_ACL_PREFIX`) 설정 seam도 추가되어 BFF JWT `groups` claim 형식에 맞출 수 있다. 운영 vocabulary는 Confluence `groupId` 기준이며 기본 우선순위는 `id,groupId,name`이다. Restriction empty 정책은 `RAG_ATLASSIAN_EMPTY_RESTRICTION_POLICY=mark_missing|allow_authenticated`으로 명시화했고, 기본값은 inherited restriction 미해석 상태에서 과다 노출을 막기 위해 `mark_missing`이다. `allow_authenticated` + `RAG_ATLASSIAN_PUBLIC_ACL_GROUP=*`는 공개 sentinel을 의도적으로 허용할 때만 opt-in으로 사용한다. `space_fallback`은 2026-06-11 결정으로 제거됐다. 남은 작업: 실제 tenant의 restriction group `id` 제공 여부 확인, 미제공 시 name→id 매핑 구현, 상위 folder/page/space permission 추가 조회 로직 확정.
