...
- 날짜: 2026-05-26
- 개정: 2026-06-02 — Confluence Admin Key 실측 결과를 반영해 항목 1의 운영 ACL 방향 보강
- 작성자: 최태성
...
**(A) `space_key` 합성 모델을 PoC 계약으로 확정**한다. 단, 2026-06-02 Admin Key 실측으로
Confluence page-level read restriction 조회가 가능함을 확인했으므로, 운영 Confluence 수집은
**(B) `allowed_groups` / `allowed_users` page-level ACL 채집**으로 전환하는 것을 후속 목표로 둔다.

구체적으로:

- PoC/샘플 ingestion은 수집 시 페이지의 `allowed_groups`를 `["space:{space_key}"]`로 합성한다
...
- payload는 결정 후에도 `space_key` + `allowed_groups` + `allowed_users`를 **모두 인덱싱**한 채로
  둔다. 수집 단계가 page-level ACL을 채우면 검색 계층은 기존 `build_acl_filter`로 그대로 적용한다.

### 근거

- 기존 PoC 근거: 샘플 데이터에는 ACL 필드가 없고, 당시 정리된 Atlassian API 문서에는
  page-level 권한 출처가 명확하지 않았다. 따라서 대안 A는 PoC에서 즉시 동작 가능한 최소 ACL이었다.
- 2026-06-02 Admin Key 실측:
  - 일반 API Token `/api/v2/pages?limit=250`: 232 pages
  - Admin Key header 포함 동일 호출: 237 pages
  - Admin Key에서만 보인 pages: 5개
  - `7798794 | 영훈없음`: 일반 직접 조회 `404`, Admin Key 직접 조회 `200`
  - `/rest/api/content/7798794/restriction/byOperation/read`에서 read 허용 user 3명 확인
  - page 조회 응답에는 권한 정보가 직접 포함되지 않고, restriction API 별도 호출이 필요함을 확인
...
- PoC 입도(granularity)는 스페이스 단위 — 같은 스페이스 내 페이지별 접근 제어는 PoC 범위 밖이다.
- 운영 Admin Key 수집은 더 넓은 문서 집합을 확보할 수 있으므로, ACL payload 저장 없이 색인하면
  권한 누출 위험이 생긴다. page-level/상위 계층 권한을 ACL metadata로 저장해야 한다.
- rag 검색 정확도는 BFF의 JWT `groups` 형식 정합에 의존(ADR 0002). 어긋나면 모든 쿼리가
  `RETRIEVAL_EMPTY`/빈 결과. → BFF 영역(아래 "합의 불필요" 참조).

### 후속 작업

- `docs/db-schema.md` / `docs/atlassian-api.md`의 과거 "content restrictions API 없음/보류" 표현을
  Admin Key 실측 기준으로 갱신한다.
- `AtlassianSourceAdapter`에 Admin Key header와 read restriction 조회 seam을 추가한다.
- page restriction user 결과는 `allowed_users=accountId[]`로 매핑한다.
- group restriction 결과는 `allowed_groups`로 매핑한다. group id/name 중 어떤 값을 BFF `groups`
  claim과 맞출지는 backend 담당자와 합의한다.
- page-level restriction이 비어 있으나 일반 조회에서 제외되는 case가 확인됐으므로, 상위
  folder/page restriction 또는 space permission 전개 정책을 정의한다.
- ACL을 확정할 수 없는 페이지는 `PageObject.is_acl_missing` / chunking worker ACL gate로 색인 차단하거나,
  명시적 fallback 정책을 적용한다. 무음 공개 처리 금지.

### 승인 필요 여부

PoC A 유지에는 불필요. 운영 B 전환에는 backend/infra와 다음 합의가 필요하다.

- Admin Key 사용 주체와 활성화/만료 운영 방식
- BFF `groups` claim 값과 Confluence group restriction 값의 매핑 방식
- page-level restriction이 비어 있는 page의 상위 권한 계층 해석 방식
...
- 양 레포 `docs/db-schema.md` §1.4 ACL 절 — PoC A 유지 + 운영 page-level ACL 전환 방향 반영.
- 양 레포 `docs/atlassian-api.md` ACL 절 — Admin Key/read restriction 실측 결과와 DATA-04 반영.
