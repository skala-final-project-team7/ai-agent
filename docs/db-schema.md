...
| `webui_link` | string | Confluence 원본 absolute URL. Confluence `_links.webui`가 `/wiki/...` 상대경로로 오면 auth-server §2-5 `siteUrl`(`https://{site}.atlassian.net`) 기준으로 정규화한다(가능 시 `#anchor` 포함) |
...
> 검색 계층은 항상 `allowed_groups OR allowed_users`를 ACL 조건으로 사용한다. 2026-06-11
> v2.6.2 기준으로 ACL 값의 space-key 레거시는 제거됐다. 샘플/fixture 데이터는
> 공개 sentinel `allowed_groups=["*"]`, `allowed_users=[]`로 합성한다.
>
...
> 따라서 운영 Confluence 수집 경로는 아래 (B)를 사용한다.
>
> - **(A) Fixture 공개 sentinel.** 샘플 데이터는 비밀 원천이 아니므로 `allowed_groups=["*"]`로
>   합성한다. `space:{key}`는 더 이상 ACL 값으로 생성하지 않는다.
> - **(B) 운영 page-level ACL.** Confluence read restriction의 user/group 결과를
...
>   동일 sentinel을 모든 principal의 group 조건에 주입한다. `space_fallback` 정책은 제거됐다.
>
